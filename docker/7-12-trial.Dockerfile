...
# ---- base OS deps (from your history) ----
...
# ---- OPTIONAL: AMDGPU kernel driver install (NOT recommended inside a container) ----
# Containers share the host kernel, so DKMS + linux-headers usually won't work or help here.
# Install AMDGPU driver on the HOST instead.
#
# RUN apt-get update && apt-get install -y --no-install-recommends \
#       linux-headers-$(uname -r) linux-modules-extra-$(uname -r) \
#     && rm -rf /var/lib/apt/lists/*
#
# RUN wget -q https://repo.radeon.com/amdgpu-install/7.2/ubuntu/jammy/amdgpu-install_7.2.70200-1_all.deb \
#     && apt-get update && apt-get install -y --no-install-recommends ./amdgpu-install_7.2.70200-1_all.deb \
#     && rm -f amdgpu-install_7.2.70200-1_all.deb \
#     && rm -rf /var/lib/apt/lists/*
#
# RUN apt-get update && apt-get install -y --no-install-recommends amdgpu-dkms \
#     && rm -rf /var/lib/apt/lists/*
#
# RUN modinfo amdgpu || true && dkms status || true

# ---- ROCm python packages (your history uses AMD nightly index) ----
# NOTE: This is the exact URL you used. Keep as-is unless you want rel/stable.
ARG ROCM_PIP_INDEX="https://rocm.nightlies.amd.com/v2-staging/gfx950-dcgpu/"

RUN python3 -m pip install --no-cache-dir \
      --extra-index-url "${ROCM_PIP_INDEX}" \
      "rocm[libraries,devel,profilers]"

# Optional: sanity check (won't fail the build if not present / not in PATH)
RUN rocm-sdk targets || true

# ---- PyTorch ROCm wheels (your history shows two alternative version sets) ----
# Choose ONE set by changing these ARG defaults.
...
# Record final environment (like your `pip freeze`)
...
ARG SETUPTOOLS_SCM_PRETEND_VERSION
RUN rocm-sdk init
ENV CPATH=/usr/local/lib/python3.10/dist-packages/_rocm_sdk_devel/include
ENV LIBRARY_PATH=/usr/local/lib/python3.10/dist-packages/_rocm_sdk_devel/lib 
...
#RUN find /usr/local/lib -name METADATA -exec bash -lc 'echo {} && grep Provides-Extra {}' \; && pip freeze && shit
#RUN pip show -f rocm-sdk-devel && cat /usr/local/lib/python3.10/dist-packages/rocm_sdk_devel-7.12.0a20260302.dist-info/METADATA && pip freeze && shit
...
# Patch the SGLANG to not to use AITER
