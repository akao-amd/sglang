        ...
        if prev_residual is not None and use_fused:
            ...
            if fused is not None:
                ...
                if not norm_fused:
                    ...
                    # Triton fires for gfx1250 at all batch sizes and for gfx95
                    # at small batch (<=64 tokens); gfx1250 has no fp8-quant path
                    # here and intentionally falls to the plain layernorm else branch.
                ...
                if not norm_fused:
                    # Same arch split as the fused path above: gfx1250 takes
                    # the plain layernorm else branch.
                    if _use_aiter and _is_gfx95_supported:
                        x_quant, hidden_states = _fused_rmsnorm_fp8_quant(
                            hidden_states,
                            self.input_layernorm.weight,
